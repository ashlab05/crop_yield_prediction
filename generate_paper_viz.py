...
def plot_metrics_heatmap(all_metrics, output_path):
    ...
    for j in range(len(metric_names)):
        ...
        ax.text(j, best_idx, ' ★', ha='left', va='top', fontsize=8, color='#2563eb', fontweight='bold')
    ...
    for i, (bar, acc) in enumerate(zip(bars, accs)):
        ...
        if i == 0:
            ax.text(bar.get_width() - 2, bar.get_y() + bar.get_height()/2,
                    '★ BEST', va='center', ha='right', fontsize=10, fontweight='bold', color='#2563eb')
...
def plot_model_ranking_summary(all_metrics, output_path):
    ...
    for idx, (title, key, higher_better, fmt) in enumerate(panels):
        ...
        # Highlight best
        ax.get_children()[0].set_edgecolor('#2563eb')
        ...
        for i, (bar, val) in enumerate(zip(bars, s_vals)):
            ...
            if i == 0:
                ax.text(bar.get_x() + bar.get_width()/2., h / 2,
                        '★ BEST', ha='center', va='center', fontsize=12, fontweight='bold', color='#2563eb', rotation=90)
...
if __name__ == "__main__":
    ...
    # 2. Save metrics JSON
    os.makedirs("viz_for_paper", exist_ok=True)
    json_path = "viz_for_paper/all_metrics.json"
    with open(json_path, 'w') as f:
        json.dump(all_metrics, f, indent=2)
    print(f"\n✓ Saved: {json_path}")

    # 3. Generate ALL visualizations
    print("\n🎨 Generating IEEE paper visualizations (White Theme)...")
    plot_metrics_heatmap(all_metrics, "viz_for_paper/metrics_heatmap.png")
    plot_accuracy_comparison(all_metrics, "viz_for_paper/accuracy_comparison.png")
    plot_f1_precision_recall(all_metrics, "viz_for_paper/f1_precision_recall.png")
    plot_comprehensive_radar(all_metrics, "viz_for_paper/comprehensive_radar.png")
    plot_mae_rmse(all_metrics, "viz_for_paper/mae_rmse_comparison.png")
    plot_r2_comparison(all_metrics, "viz_for_paper/r2_comparison.png")
    plot_all_classification_metrics(all_metrics, "viz_for_paper/all_metrics_grouped_bar.png")
    plot_model_ranking_summary(all_metrics, "viz_for_paper/model_ranking_summary.png")
    
    print("\n📊 Generating individual metric bar charts...")
    plot_individual_metrics(all_metrics, "viz_for_paper")

    print("\n" + "=" * 60)
    print(f"ALL DONE! Generated all IEEE-grade paper plots + all_metrics.json in 'viz_for_paper'")
    print("=" * 60)
